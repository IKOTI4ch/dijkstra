...
type queue interface {
	PushOrdered(*vertices.Vertex)
	PopOrdered() *vertices.Vertex
	Len() int
}

// PriorityQueueNewShort creates a new priority queue for short solving
func priorityQueueNewShort() queue {
...
func priorityQueueNewLong() queue {
...
type Item struct {
	value *vertices.Vertex // The value of the item; arbitrary.
}
...
func (pq *priorityQueueBase) Push(v *vertices.Vertex) {
	/*	id := v.(*vertices.Vertex).ID
		for _, v := range *pq {
			if v.value.ID == id {
				return
			}
...
func (pq *priorityQueueWrapper) PushOrdered(v *vertices.Vertex) {
	pq.Push(v)
	pq.up(pq.Len() - 1)
}

func (pq *priorityQueueWrapper) PopOrdered() *vertices.Vertex {
	n := pq.Len() - 1
	pq.Swap(0, n)
	pq.down(0, n)
	return pq.Pop()
...
func (pq *priorityQueueBase) Pop() *vertices.Vertex {
	old := *pq
	n := len(old)
	item := old[n-1]
	*pq = old[0 : n-1]
