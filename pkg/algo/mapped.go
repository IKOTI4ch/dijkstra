...
// GetMapped gets the key associated with the mapped int
...
// AddMappedArc adds a new Arc from Source to Destination, for when vertices are
...
func (g *Graph) RemoveArc(Source, Destination int) error {
	...
	g.Vertices[Source].RemoveArc(Destination)
